...
class Exam extends CActiveRecord {
    ...
    public function getExamBySubjectId($level_id,$subject_id) {
        $command = Yii::app()->db->createCommand();
            $result = $command->select('*')->from('esto_exam')->where('status=:status AND level_id like :level_id AND subject_id like :subject_id', array(':status' => 1, ':level_id' => $level_id, ':subject_id' => $subject_id))->order('sort_order')->queryAll();
            return $result;
    }
