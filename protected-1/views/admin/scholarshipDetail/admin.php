<?php
$this->breadcrumbs=array(
	'ข้อมูลทุนการศึกษา'=>array('index'),
	'จัดการข้อมูล',
);

$this->menu=array(
	//array('label'=>'List ScholarshipDetail', 'url'=>array('index')),
	array('label'=>'เพิ่มข้อมูลทุนการศึกษา', 'url'=>array('create')),
);

Yii::app()->clientScript->registerScript('search', "
$('.search-button').click(function(){
	$('.search-form').toggle();
	return false;
});
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('scholarship-detail-grid', {
		data: $(this).serialize()
	});
	return false;
});
");
?>

<h1>จัดการข้อมูลทุนการศึกษา</h1>

<p>
You may optionally enter a comparison operator (<b>&lt;</b>, <b>&lt;=</b>, <b>&gt;</b>, <b>&gt;=</b>, <b>&lt;&gt;</b>
or <b>=</b>) at the beginning of each of your search values to specify how the comparison should be done.
</p>

<?php echo CHtml::link('Advanced Search','#',array('class'=>'search-button')); ?>
<div class="search-form" style="display:none">
<?php $this->renderPartial('_search',array(
	'model'=>$model,
)); ?>
</div><!-- search-form -->

<?php $this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'scholarship-detail-grid',
	'dataProvider'=>$model->search(),
	'filter'=>$model,
	'columns'=>array(
                array(
			'name'=>'scholar_id',
			'htmlOptions'=>array('style'=>'text-align: left;width: 50px;'),
		),
		'name',
                array(
			'name'=>'period_start',
                        'value'=>'date(\'d/m/Y\',strtotime($data->period_start))',
			'htmlOptions'=>array('style'=>'text-align: left;width: 100px;'),
		),
                array(
			'name'=>'period_end',
                        'value'=>'date(\'d/m/Y\',strtotime($data->period_end))',
			'htmlOptions'=>array('style'=>'text-align: left;width: 120px;'),
		),
                 array(
			'name'=>'price',
			'htmlOptions'=>array('style'=>'text-align: left;width: 50px;'),
		),
		array(
			'name'=> 'status',
			'value'=> '($data->status)? \'Enabled\' : \'Disabled\'',
			'htmlOptions'=>array('style'=>'text-align: center; width: 60px;'),
                        'filter'=>array('1'=>'Enabled','0'=>'Disabled'),
		),
		array(
			'class'=>'CButtonColumn',
			'template'=>'{update}&nbsp;&nbsp;{delete}',
			'headerHtmlOptions'=>array('style'=>'width:40px;'),
                        'htmlOptions' => array('style'=>'width:40px; text-align:center'),

		),
	),
)); ?>
